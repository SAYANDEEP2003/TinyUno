#define FUNC_READ 1
#define FUNC_WRITE 1
#define BAUD_RATE 115200  // or the correct baud rate
#define F_CPU 16000000UL  // or the correct CPU frequency
/**********************************************************/
...
int main(void) {
  ...
  spi_init();
    ...
    uint32_t partition_start = find_fat32_partition();
  
    // Find root directory
    uint32_t root_dir_sector = find_root_directory(partition_start);
  
    // Find HEX file in root directory
    uint32_t first_cluster = find_hex_file(root_dir_sector);
  
    // If HEX file found, read and flash it
    if (first_cluster != 0) {
        read_and_flash_hex(first_cluster);
    }
